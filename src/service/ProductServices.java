    ...
    public ObservableList<SalesItem> loadData(){
        ...
        try {
            ...
            while (rsLoadAgencyProduct.next()) {
                salesBakeryData.add(new SalesItem(UtilityMethod.addPrefix("AP", rsLoadAgencyProduct.getString(1)), rsLoadAgencyProduct.getString(2), rsLoadAgencyProduct.getString(4), rsLoadAgencyProduct.getFloat(7), "Agency Product", "Available", rsLoadAgencyProduct.getString(12)));
            }
        ...
        searchTextField.textProperty().addListener((observable,oldValue,newValue) ->{
            filteredData.setPredicate(salesItem -> {
                ...
                if(salesItem.getsIPID().toLowerCase().indexOf(lowerCaseFilter) != -1){
                    ...
                    //return if filter matches data
                    return true;
                }else if(String.valueOf(salesItem.getsIPBarcode()).toLowerCase().indexOf(lowerCaseFilter) !=-1) {
                    return true;
                }else{
                    //have no matchings
                    return false;
