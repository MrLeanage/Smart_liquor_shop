...
public class Notification extends Thread{
    ...
    private void setKeyFrames(){
        if(UserAuthentication.getAuthenticatedSession().getuType().equals("Admin")){
            keyFrameObservableList.add(agencyProductDiscontinueAlert(30));
            keyFrameObservableList.add(lowStock(30));
            keyFrameObservableList.add(outOfStock(30));
            keyFrameObservableList.add(agencyProductExpireAlert(30));
            keyFrameObservableList.add(stockItemExpireAlert(30));

        }
        if(UserAuthentication.getAuthenticatedSession().getuType().equals("Supervisor")){
            keyFrameObservableList.add(newOrderNotification(30));
    ...
    private void alertNotificationStage(String title, String text, ActionEvent action){
        ...
        NotificationManager notificationBuilder = NotificationManager.create().create()
                .title(title)
                .text(text)
                .graphic(null)
        ...
        notificationBuilder.redStyle();
        notificationBuilder.showWarning();

    }
    private void errorNotificationStage(String title, String text, ActionEvent action){

        NotificationManager notificationBuilder = NotificationManager.create()
                .title(title)
                .text(text)
                .graphic(null)
                .hideAfter(Duration.seconds(10))
